...
window.RoutingSystem = class {
  ...
  processHighwayData(data) {
    ...
    for (const element of data.elements) {
      if (element.type === 'way' && element.nodes) {
        const coordinates = element.nodes.map(id => [nodes[id].lon, nodes[id].lat]);
  ...
  async renderHighwayNetwork() {
    try {
      ...
      window.log(`Highway network rendered with ${highwayFeatures.length} segments`);
  ...
  async findShadiestPath(startCoordinates, endCoordinates) {
    try {
      ...
      if (!startNodeId || !endNodeId) {
        throw new Error('Could not find nearest graph nodes for start/end points');
      }

      // Run Dijkstra's algorithm
      const pathNodeIds = this.dijkstra(graph.nodes, graph.edges, startNodeId, endNodeId);
      ...
      window.log(`Shady path found. Average shade: ${window.formatNumber(pathStats.avgShade * 100)}%`);
